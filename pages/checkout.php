...
<?php if (class_exists('STM_LMS_Templates')) {
    STM_LMS_Templates::show_lms_template('pmpro/checkout-warning', array(
...
<div id="pmpro_level-<?php echo esc_attr($pmpro_level->id); ?>">
    <form id="pmpro_form" class="pmpro_form"
          action="<?php if (!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>"
          method="post">
        ...
        <?php if ($pmpro_msg) {
            ...
            <div id="pmpro_message"
                 class="pmpro_message <?php echo esc_attr($pmpro_msgt); ?>"><?php echo wp_kses_post($pmpro_msg); ?></div>
            <?php
        } else {
            ?>
        ...
        <?php if ($pmpro_review) { ?>
            <p><?php _e('Almost done. Review the membership information and pricing below then <strong>click the "Complete Payment" button</strong> to finish your order.', 'masterstudy'); ?></p>
        <?php } ?>
        ...
        <table id="pmpro_pricing_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
            ...
            <tr>
                <th>
                    <h3 class="pmpro_thead-name"><?php _e('Membership Level', 'masterstudy'); ?></h3>
                    <?php if (count($pmpro_levels) > 1) { ?><span class="pmpro_thead-msg">
                        <a href="<?php echo pmpro_url("levels"); ?>"><?php _e('Change level', 'masterstudy'); ?></a>
                        </span><?php } ?>
                </th>
            </tr>
            </thead>
            ...
            <tr>
                <td class="lp-pmpro-td">
                    <div class="lp-pmpro-name">
                        <h4><?php echo sanitize_text_field($pmpro_level->name); ?></h4>
                    </div>
                    <div class="lp-pmpro-content">
                        <?php echo '<div class="lp-pmpro-desc"><label>' . esc_html__('Membership level:', 'masterstudy') . '</label>' . $pmpro_level->name . '</div>'; ?>

                        <?php if ($discount_code && pmpro_checkDiscountCode($discount_code)) { ?>
                            <?php printf(__('<p class="pmpro_level_discount_applied">The <strong>%s</strong> code has been applied to your order.</p>', 'masterstudy'), $discount_code); ?>
                        <?php } ?>

                        <div id="pmpro_level_cost">

                            <?php echo '<div class="lp-pmpro-desc lp-pmpro-price"><label>' . esc_html__('Price:', 'masterstudy') . '</label>' . pmpro_getLevelCost($pmpro_level, true, true) . '</div>'; ?>

                            <?php echo '<div class="lp-pmpro-desc"><label>' . esc_html__('Expires after:', 'masterstudy') . '</label>' . sprintf(__("%d %s", 'masterstudy'), $pmpro_level->expiration_number, pmpro_translate_billing_period($pmpro_level->expiration_period, $pmpro_level->expiration_number)) . '</div>'; ?>
                            ...
                            <?php if ($discount_code && !$pmpro_review) { ?>
                                <p id="other_discount_code_p" class="pmpro_small">
                                    <a id="other_discount_code_a"
                                       href="#discount_code"><?php _e('Click here to change your discount code', 'masterstudy'); ?></a>.
                                </p>
                            <?php } elseif (!$pmpro_review) { ?>
                                <p id="other_discount_code_p"
                                   class="pmpro_small"><?php _e('Do you have a discount code?', 'masterstudy'); ?>
                                    <a id="other_discount_code_a"
                                       href="#discount_code"><?php _e('Click here to enter your discount code', 'masterstudy'); ?></a>.
                                </p>
                            <?php } elseif ($pmpro_review && $discount_code) { ?>
                                <p>
                                    <strong><?php _e('Discount Code', 'masterstudy'); ?>
                                        :</strong> <?php echo sanitize_text_field($discount_code); ?>
                                </p>
                            <?php } ?>
                        ...
                        if (!empty($level_description))
                            echo '<div class="lp-pmpro-desc lp-pmpro-description"><label>' . esc_html__('Description:', 'masterstudy') . '</label>' . $pmpro_level->description . '</div>';
                        ?>
                    </div>
                </td>
            </tr>
        ...
        <?php if (!$skip_account_fields && !$pmpro_review) { ?>
            <table id="pmpro_user_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0"
                ...
                <tr>
                    <th>
                        <h3 class="pmpro_thead-name"><?php _e('Account Information', 'masterstudy'); ?></h3>
                        <span class="pmpro_thead-msg"><?php _e('Already have an account?', 'masterstudy'); ?>
                            <a href="<?php echo add_query_arg('redirect_to', pmpro_url("checkout", "?level=" . $pmpro_level->id), STM_LMS_User::login_page_url()); ?>"><?php _e('Login here', 'masterstudy'); ?></a>
						</span>
                    </th>
                </tr>
                </thead>
                ...
                <tr class="lp-pmpro-account-info">
                    <td class="lp-pmpro-td">
                        <div class="lp-pmpro-desc">
                            <label for="username"><?php _e('Username', 'masterstudy'); ?></label>
                        ...
                        <div class="lp-pmpro-desc">
                            <label for="password"><?php _e('Password', 'masterstudy'); ?></label>
                        ...
                        if ($pmpro_checkout_confirm_password) {
                            ?>
                            <div class="lp-pmpro-desc">
                                <label for="password2"><?php _e('Confirm Password', 'masterstudy'); ?></label>
                        ...
                        if ($pmpro_checkout_confirm_email) {
                            ?>
                            <div class="lp-pmpro-desc">
                                <label for="bconfirmemail"><?php _e('Confirm E-mail Address', 'masterstudy'); ?></label>
                        ...
                        <div class="pmpro_hidden">
                            <label for="fullname"><?php _e('Full Name', 'masterstudy'); ?></label>
                            <input id="fullname" name="fullname" type="text"
                                   class="input <?php echo pmpro_getClassForField("fullname"); ?>" size="30" value=""/>
                            <strong><?php _e('LEAVE THIS BLANK', 'masterstudy'); ?></strong>
                        </div>
        ...
        <?php } elseif ($current_user->ID && !$pmpro_review) {
            ...
            <p id="pmpro_account_loggedin" class="message message-notice">
                <?php printf(__('You are logged in as <strong>%s</strong>. If you would like to use a different account for this membership, <a href="%s">%s</a>.', 'masterstudy'), $current_user->user_login, $url, $text); ?>
            </p>
        <?php } ?>
        ...
        <?php if (pmpro_getGateway() == "paypal" && empty($pmpro_review)) { ?>
            <table id="pmpro_payment_method" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0"
                ...
                <thead>
                <tr>
                    <th><?php _e('Choose your Payment Method', 'masterstudy'); ?></th>
                </tr>
                </thead>
                <tbody>
                <tr>
                    <td>
						...
						<span class="gateway_paypal">
							...
							<a href="javascript:void(0);"
                               class="pmpro_radio"><?php _e('Check Out with a Credit Card Here', 'masterstudy'); ?></a>
						</span>
							...
							<a href="javascript:void(0);"
                               class="pmpro_radio"><?php _e('Check Out with PayPal', 'masterstudy'); ?></a>
						</span>
                        </div>
                    </td>
                </tr>
        ...
        if ($pmpro_include_billing_address_fields) {
            ...
            <table id="pmpro_billing_address_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0"
                ...
                <thead>
                <tr>
                    <th><?php _e('Billing Address', 'masterstudy'); ?></th>
                </tr>
                </thead>
                <tbody>
                <tr>
                                    ...
                                    <select name="bcountry" class=" <?php echo pmpro_getClassForField("bcountry"); ?>">
                                        ...
                                        foreach ($pmpro_countries as $abbr => $country) {
                                            ...
                                            <option value="<?php echo esc_attr($abbr); ?>"
                                                    <?php if ($abbr == $bcountry) { ?>selected="selected"<?php } ?>><?php echo sanitize_text_field($country); ?></option>
                                            <?php
                                        }
                                        ?>
                                    </select>
        ...
        if ($pmpro_include_payment_information_fields) {
            ...
            <table id="pmpro_payment_information_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0"
                ...
                <tr>
                    <th>
                        <span class="pmpro_thead-name"><?php _e('Payment Information', 'masterstudy'); ?></span>
                        <span class="pmpro_thead-msg"><?php printf(__('We Accept %s', 'masterstudy'), $pmpro_accepted_credit_cards_string); ?></span>
                    </th>
                </tr>
                </thead>
                <tbody>
                        ...
                        <div class="stm_lms_pmpro_payment_info">
                            ...
                            if ($pmpro_show_cvv) { ?>
                                <div class="pmpro_payment-cvv">
                                    ...
                                    <small>(<a href="javascript:void(0);"
                                               onclick="javascript:window.open('<?php echo pmpro_https_filter(PMPRO_URL) ?>/pages/popup-cvv.html','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php _e("what's this?", 'masterstudy'); ?></a>)
                                    </small>
                                </div>
                            <?php } ?>
                            ...
                            <?php if ($pmpro_show_discount_code) { ?>
                                <div class="pmpro_payment-discount-code">
                                    <label for="discount_code"><?php _e('Discount Code', 'masterstudy'); ?></label>
        ...
        if ($tospage && !$pmpro_review) {
            ...
            <table id="pmpro_tos_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0"
                ...
                <thead>
                <tr>
                    <th><?php echo sanitize_text_field($tospage->post_title); ?></th>
                </tr>
                </thead>
                <tbody>
                <tr class="odd">
                    <td>
                        ...
                        <label class="pmpro_normal pmpro_clickable"
                               for="tos"><?php printf(__('I agree to the %s', 'masterstudy'), $tospage->post_title); ?></label>
                    </td>
                </tr>
                </tbody>
            </table>
        ...
        <div class="pmpro_submit">
            <?php if ($pmpro_review) { ?>
				...
				<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout"
                       value="<?php _e('Complete Payment', 'masterstudy'); ?> &raquo;"/>
			</span>
            ...
            <?php } else { ?>
                ...
                if ($pmpro_checkout_default_submit_button) {
					...
					<input type="submit" class="btn btn-default" value="<?php if ($pmpro_requirebilling) {
                        _e('Submit and Check Out', 'masterstudy');
                    } else {
                        _e('Submit and Confirm', 'masterstudy');
                    } ?>"/>
				</span>
                    <?php
                }
